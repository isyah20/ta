            ...
            <div class="row justify-content-center">
                ...
                <div class="col-11 col-lg-8 bg-white rounded-3 p-3 mx-3 mb-5" id="form-profile-tab">
                    ...
                    <div id="profile2" class="p-3 col-lg-11 mx-auto" x-data="userProfile">
                        <form action="<?= base_url('lengkapi-profile') ?>" method="post" class="form-profile profile" autocomplete="off">
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <input type="text" name="nama" id="nama" x-model="companyName" @keyup="validateCompanyName()" :class="errors.companyName ? 'form-control is-invalid' : 'form-control'">
                            ...
                            <div class="mb-3 row input-npwp" style="display: none;">
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <input type="text" name="npwp" id="npwp" x-model="npwp" x-mask="99.999.999.9-999.999" placeholder="##.###.###.#-###.###" @keyup="validateNpwp()" :class="errors.npwp ? 'form-control is-invalid' : 'form-control'" disabled="true">
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <input type="email" class="form-control" name="email" id="email" readonly disabled>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <div class="d-flex">
                                        <div style="width: 65% !important;">
                                            <input type="text" :class="errors.phoneNumber ? 'form-control is-invalid' : 'form-control'" name="no_telp" id="noTelp" maxlength="20" x-model="phoneNumber" autocomplete="off" @keyup="validatePhoneNumber()">
                                        ...
                                        <div class="ms-2">
                                            <button class="btn btn-success" id="btn-verifikasi" x-text="loading ? 'Mengirim...' : 'Verifikasi WhatsApp'" @click="submitVerify($event)"></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <textarea id="alamat" name="alamat" x-model="address" @keyup="validateAddress()" :class="errors.address ? 'form-control mb-2 is-invalid' : 'form-control mb-2'"></textarea>
                                    <div class="d-flex">
                                        <div class="me-2">
                                            <select class="form-select select2-provinsi" id="provinsi" name="provinsi" x-model="province" x-ref="select2-province">
                                                <option value="">Provinsi</option>

                                                <?php foreach ($provinsi['rajaongkir']['results'] as $value) : ?>
                                                    <option value="<?= $value['province_id'] ?>"><?php echo $value['province'] ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="me-2">
                                    ...
                                    <p class="small text-danger mt-2" x-show="errors.province" style="display: none;" x-text="msg.province">Provinsi wajib diisi!</p>
                                    <p class="small text-danger mt-2" x-show="errors.city" style="display: none;" x-text="msg.city">Kabupaten wajib diisi!</p>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-8 col-xl-9">
                                    <button class="btn btn-danger px-3 m-1 btn-update" x-text="loading ? 'Menyimpan...' : 'Perbarui'" @click="submitForm($event)">Perbaharui</button>
...
<script>
    var kategori, wa_status, countdown, timeLeft;
    ...
    $(document).ready(function() {
        $.ajax({
            ...
            success : function(data){
                ...
                if (kategori != '4') $('.input-perusahaan, .input-npwp').show(); else $('.input-perusahaan, .input-npwp').hide();
                ...
                $('#noTelp').val(data.no_telp).prop('disabled', wa_status);
                $('#btn-verifikasi').prop('disabled', wa_status).text(btn_verif);
                
                let alamat = data.alamat.split(',');
                let provinsi = alamat[alamat.length - 1].trim();
                let kabupaten = alamat[alamat.length - 2].trim();
                alamat = alamat.splice(alamat.length - 1, 1).splice(alamat.length - 2, 1).join(' ');
                $('#alamat').text(alamat);
        ...
        const provinceToId = (name, provinces) => {
            const selected = provinces.filter(value => value.wilayah == name);
            if (selected.length > 0) return selected[0].id_wilayah;
            return 0;
        }

        const cityToId = (cityName, listCity) => {
            const selected = listCity.filter(value => value.text.trim() == cityName);
            if (selected.length > 0) return parseInt(selected[0].id);
            return 0;
        }
        ...
        const splitAddr = (addr) => {
            ...
            let listAddr = addr.split(',');
            address = listAddr.map(val => val.trim());
            const province = address[address.length - 1];
            if (address.length < 2) {
                return {
                    city: '',
                    province: province
        ...
        const selectCity = (provinceId, cityName) => {
            $.ajax({
                url: `${base_url}getcity/${provinceId}`
            }).done(resp => {
                let data = resp.reduce((acc, item) => [...acc, {
                    id: item.id_wilayah,
                    text: item.wilayah
                }], [])
                ...
                const cityId = cityToId(cityName, data);
                if (cityId > 0) {
                    Alpine.store('city', cityName);
                    $('.select2-kabupaten').val(cityId).trigger('change');
                }
            }).fail(err => console.log(err))
        }

        // Load data provinsi dan select provinsi default dari user
        // TODO: satukan ke alpine agar tidak terjadi keterlambatan saat alpine.store menyimpan data user yang ada alamatnya.
        $.ajax({
            url: `${base_url}getprovince`,
        }).done(resp => {
            const selectProv = $('.select2-provinsi');
            selectProv.empty();
            
            $.each(resp, function(index, option) {
                selectProv.append($('<option>').text(option.wilayah).attr('value', option.id_wilayah));
            })
            
            $('.select2-provinsi').select2();

            setTimeout(() => {
                const store = Alpine.store('users');
                const {
                    city,
                    province
                } = splitAddr(store.alamat)
                
                if (province.length > 0) {
                    Alpine.store('province', province);
                    
                    const id = provinceToId(province, resp);
                    if (id > 0) {
                        selectProv.val(id).trigger('change');
                        selectCity(id, city);
                    }
                }
            }, 500)
    ...
    function startCountdown() {
        ...
        countdown = setInterval(function() {
            ...
            countdownElement.html("Kirim ulang: " + minutes + " menit " + seconds + " detik");
