...
#scrap paket
'''cursor.execute("SELECT id_lpse,url FROM lpse WHERE id_lpse=11 ORDER BY id_lpse")
result = cursor.fetchall()
for paket in result:
    lpse = Lpse(paket[1].replace('/eproc4',''))
    daftar_paket = lpse.get_paket_tender()
    jum_data = daftar_paket['recordsTotal']
    daftar_paket = lpse.get_paket_tender(start=0, length=jum_data, tahun=2022, data_only=True)
    tender = {}
    
    for data in daftar_paket:
        tender['id_lpse'] = paket[0]
...
'''            
#scrap detail paket
cursor.execute("SELECT lpse.id_lpse,url,kode_tender FROM paket,lpse WHERE paket.id_lpse=lpse.id_lpse AND nama_tender IS NULL AND id_paket BETWEEN 160000 AND 190000 ORDER BY RAND()")
result = cursor.fetchall()
