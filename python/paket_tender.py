...
cursor.execute("SELECT id_lpse,url FROM lpse WHERE id_lpse BETWEEN 740 AND 1063 AND id_lpse NOT IN (SELECT id_lpse FROM history_scrap WHERE DATE(tgl_scrap)=CURRENT_DATE) ORDER BY RAND()")
result = cursor.fetchall()
for paket in result:
    lpse = Lpse(paket[1].replace('/eproc4',''))
    daftar_paket = lpse.get_paket_tender()
    jum_paket = daftar_paket['recordsTotal']
    tahun_paket = 2023
    daftar_paket = lpse.get_paket_tender(start=0, length=jum_paket, tahun=tahun_paket, data_only=True)
    # tender = {}
    id_lpse = str(paket[0])
    id_lpse_scraped = '0'
    
    for data in daftar_paket:
        # tender['id_lpse'] = id_lpse
        # tender['kode_tender'] = data[0]
        
        detil = lpse.detil_paket_tender(id_paket=data[0])
        detil.get_all_detil()
        tender = detil.todict()
        pengumuman = {}
        jadwal = {}
        ...
        #simpan paket
        '''try:
            sql = "SELECT kode_tender FROM paket WHERE kode_tender='"+data[0]+"'"
            cursor.execute(sql)
            result = cursor.fetchone()
            
            if result is None:
                sql = ("INSERT INTO paket (id_paket,id_lpse,kode_tender) VALUES (NULL, %(id_lpse)s, %(kode_tender)s)")
            else:
                sql = ("UPDATE paket SET id_lpse=%(id_lpse)s WHERE kode_tender=%(kode_tender)s")
        ...
        if id_lpse_scraped != id_lpse:
            try:
                sql = "SELECT id_history FROM history_scrap WHERE id_lpse="+id_lpse
                cursor.execute(sql)
                result = cursor.fetchone()
                
                if result is None:
                    sql = ("INSERT INTO history_scrap VALUES (NULL,"+id_lpse+","+str(jum_paket)+",CURRENT_TIMESTAMP)")
                else:
                    id_history = str(result[0])
                    sql = ("UPDATE history_scrap SET jumlah_tender="+str(jum_paket)+",tgl_scrap=CURRENT_TIMESTAMP WHERE id_history="+id_history)
                
                cursor.execute(sql)
                koneksi.commit()
                id_lpse_scraped = id_lpse
            except Exception as e:
                print("Error: ", e)
                koneksi.rollback()
