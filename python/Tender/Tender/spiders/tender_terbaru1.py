...
class TenderTerbaru(scrapy.Spider):
    ...
    def start_requests(self):
        self.cursor.execute("SELECT CONCAT(url,'#',id_lpse) AS url FROM lpse WHERE id_lpse NOT IN (SELECT id_lpse FROM tender_terbaru WHERE DATE(tgl_update)=CURRENT_DATE GROUP BY id_lpse) AND id_lpse=323 ORDER BY RAND()")
